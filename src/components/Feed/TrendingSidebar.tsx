...
const TrendingSidebar = ({ onSelectTopic, activeTopic }: TrendingSidebarProps) => {
            ...
            topics.map((topic, index) => {
              ...
              return (
                <div
                  ...
                  className={`flex items-center gap-3 p-2 rounded-lg transition-colors cursor-pointer border border-transparent ${
                    isActive
                      ? "bg-accent/10 border-accent/30 text-accent"
                      : "hover:bg-muted/50"
                  }`}
                >
                  <div className={`p-2 rounded-lg ${isActive ? "bg-accent/20" : "bg-accent/10"}`}>
                  <TrendingUp className="h-4 w-4 text-accent" />
                </div>
                <div className="flex-1">
                    <p className={`font-medium text-sm ${isActive ? "text-accent" : ""}`}>{topic.name}</p>
                </div>
